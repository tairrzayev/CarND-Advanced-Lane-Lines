...
class LaneDetector:
    ...
    def find_lane_pixels(self, binary_warped):
        ...
        if self.debug_enabled:
            out_binary = np.zeros(binary_warped.shape, np.uint8)
            out_binary[binary_warped != 0] = 255
            debug_img = np.dstack((out_binary, out_binary, out_binary))
        ...
        if debug_img is not None:
            debug_img[lefty, leftx] = [255, 0, 0]
            debug_img[righty, rightx] = [0, 0, 255]
    ...
    def search_around_previous_fit(self, binary_warped):
        ...
        # Draw the lane onto the warped blank image
        result = None
        if debug_img is not None and window_img is not None:
            cv2.fillPoly(window_img, np.int_([left_line_pts]), (0, 255, 0))
            cv2.fillPoly(window_img, np.int_([right_line_pts]), (0, 255, 0))
            result = cv2.addWeighted(debug_img, 1, window_img, 0.3, 0)
    ...
    def draw_lane_data(self, image, ploty):
        curv_rad = self.calculate_curve_radius(ploty)

        cv2.putText(image, 'Radius of Curvature = {:.2f}(m)'.format(curv_rad), (32, 64), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255), 2, cv2.LINE_AA)

        dist_from_lane_center, direction = self.calculate_distance_from_the_lane_center(image)
    ...
    def calculate_curve_radius(self, ploty):
    ...
    def calculate_distance_from_the_lane_center(self, image):
        h = image.shape[0]
        w = image.shape[1]
        ...
        return dist_from_lane_center, direction
